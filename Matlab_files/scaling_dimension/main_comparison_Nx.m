...
options = odeset('RelTol',1e-8,'AbsTol',1e-8);%,'MaxStep',0.1);  % accuracy of ODE solver
...
Dim = [4,6,8];  % considered Dimensions (truncation of the KL expansion)
MM = [50,100,200];  % considered ensemble sizes

ell = 6;          % discretization of the PDE
I = 2^ell;  
xx_b = linspace(0,1,I+1);       % domain with boundary
xx = xx_b(2:end-1);             % domain
...
rand_ref = randn(max(Dim),1);

for N_x = Dim % dimension of target distribution
    m=1;
    for M = MM % ensemble size
        ...
        P0 = eye(N_x,N_x)*D;     % prior covariance
        ...
        Pxu_t = 1/M*(X_est1-x_quer)*(u(X_est1)-uquer)';

        % force which pushes the particle system into MAP direction
        grad_V_1 = -(Pxu_t*(R\(u(X_est1)-y)))-P_t*(P0\(X_est1-m0));
        ...
        Pxu_t = 1/M*(X_est2-x_quer)*(u(X_est2)-uquer)';
        
        % force which pushes the particle system into MAP direction
        grad_V_1 = -(Pxu_t*(R\(u(X_est2)-y)))-P_t*(P0\(X_est2-m0));
        % force which spreads the particles
        grad_V_2 = (P_t*(K1)+P_t*(K2));
        ...
        str = sprintf('$B_*, N_x=%d, M=%d$',N_x,M);
        title(str,'Interpreter','latex','FontSize',20)
        ...
        Pxu_t = 1/M*(X_est3-x_quer)*(u(X_est3)-uquer)';
         % force which pushes the particle system into MAP direction
        grad_V_1 = -(Pxu_t*(R\(u(X_est3)-y)))-P_t*(P0\(X_est3-m0));
        % force which spreads the particles
        grad_V_2 = (P_t*(K1)+P_t*(K2));
        ...
        str = sprintf('$B_t, N_x=%d, M=%d$',N_x,M);
        title(str,'Interpreter','latex','FontSize',20)
        ...
        p2 = plot(xx_b,[0;u_3_est_mean;0],'-.','Color',[0.5 0 0],'LineWidth',3,'DisplayName','estimate');hold on
